...
static void init_ps(const char *str,
		    int  is_epsf)
{
	time_t ltime;
	char tstr[41];
...
void close_output_file(bool report)
{
	...
	if (fout == 0)
		return;
	if (tunenum == 0)
		alert("No tunes written to output file");
	close_page();
	...
	fclose(fout);
	if (report)
		alert( "Output written on %s (%d page%s, %ld bytes)",
			outfnam, nbpages, nbpages == 1 ? "" : "s", m);
	fout = 0;
	file_initialized = 0;
	nbpages = tunenum = 0;
}
...
static void format_hf(const char *p)
{
	char *q;
	char s[256];
	time_t ltime;
...
{
	if ((fout = fopen(out_file, "w")) == 0) {
		alert( "Cannot create output file %s", out_file);
	}
	outfnam = const_cast<char*>(out_file);
}
...
void write_eps(void)
{
	...
	if (choose_outname) {
		...
		epsf_title(fnm);
		strcat(fnm, ".eps");
	} else	sprintf(fnm, "%.70s%03d.eps", "out", ++nepsf);
	sprintf(finf, "%.72s (%.4s)", in_fname, info.xref);
	if ((fout = fopen(fnm, "w")) == 0) {
		alert( "Cannot open output file %s", fnm);
		return;
	}
	init_ps(finf, 1);
	fprintf(fout, "0 %.1f T\n", -bposy);
...
void a2b(void)
{
	...
	if (nbuf >= BUFFSZ - 500) {	/* must have place for 1 more line */
		alert( "a2b: buffer full, BUFFSZ=%d", BUFFSZ);
		return;
	}
...
{
	if (ln_num >= BUFFLN) {
		alert("max number of buffer lines exceeded -- check BUFFLN");
		return;
	}
...
void check_buffer(void)
{
	if (nbuf + BUFFSZ1 > BUFFSZ) {
		alert("Possibly bad page breaks, BUFFSZ exceeded");
		write_buffer();
		use_buffer = 0;
	}
}
