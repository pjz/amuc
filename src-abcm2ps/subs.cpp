...
{
	alert("This cannot happen! Internal error: %s", msg);
	if (fatal) {
		alert("Emergency stop");
		exit(3);
	}
	alert("Trying to continue...");
}
...
float scan_u(char *str)
{
	...
	alert("++++ Unknown unit value '%s'", str);
	return 20 * PT;
}
...
static void put_str3(const char *head,
		     const char *str,
		     const char *tail)
{
	PUT0(head);
	      ...
	      int type)
{
	...
	t = (struct text *) malloc(sizeof (struct text) - 2
				     + strlen(s) + 1);
	strcpy(t->text, s);
...
void write_title(int i)
{
	...
	if (i == 0 && cfmt.withxrefs)
		PUT1("%s.  ", info.xref);
	put_str(t);
	if (cfmt.titleleft || i>0)  // subtitle always leftside
		PUT0(") 0 0 M show\n");
