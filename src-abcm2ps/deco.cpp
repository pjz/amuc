...
static struct deco_elt {
	deco_elt *next;	/* next decoration */
	SYMBOL *s;	/* symbol */
	uchar t;		/* decoration index */
	uchar staff;	/* staff */
...
struct deco_def_s;
typedef void draw_f(deco_elt *de);
...
static struct deco_def_s {
	const char *name;
	uchar func;	/* function index */
	signed char ps_func;	/* postscript function index */
	char h;	/* height */
	char wl;	/* width */
	char wr;
	uchar str;	/* string index - 127=deco name */
} deco_def_tb[128] = {
	{0, 0, 0, 0},			/* 0: unknown */
	{"dot", 0, 0, 4},		/* 1 */
	{"roll", 3, 10, 10},		/* 2 */
...
static const char *ps_func_tb[64] = {
	"stc",		/* 0: dot */
	"emb",		/* 1: tenuto */
	"sld",		/* 2: slide */
	"grm",		/* 3: gracing mark */
...
static const char *str_tb[32] = {
	0,
	"FINE",		/* 1 */
};

static SYMBOL *first_note;	/* first note/rest of the line */

static void draw_gchord(SYMBOL *s, float gchy);

/* get the max/min vertical offset */
static float get_y(SYMBOL *s,
		   int up,
		   float x,
		   float w,
		   float h)
...
static void d_cresc(deco_elt *de)
{
	SYMBOL *s;
	deco_def_s *dd;
	int staff, voice, up;
...
static void d_near(deco_elt *de)
{
	SYMBOL *s;
	deco_def_s *dd;
	int y, sig;
...
static void d_slide(deco_elt *de)
{
	SYMBOL *s;
	deco_def_s *dd;
	int m;
...
static void d_trill(deco_elt *de)
{
	SYMBOL *s;
	deco_def_s *dd;
	int staff, voice, up;
...
static void d_upstaff(deco_elt *de)
{
	SYMBOL *s;
	deco_def_s *dd;
	float x, yc;
...
void deco_add(char *text)
{
	...
	if (sscanf(text, "%15s %d %15s %d %d %d%n",
		...
		alert( "Invalid deco %s", text);
		return;
	}
	if (c_func < 0 || c_func >= sizeof func_tb / sizeof func_tb[0]) {
		alert( "%%%%deco: bad C function index (%s)", text);
		return;
	}
	if (h < 0 || wl < 0 || wr < 0) {
		alert( "%%%%deco: cannot have a negative value (%s)", text);
		return;
	}
	if (h > 50 || wl > 30 || wr > 30) {
		alert( "%%%%deco: abnormal h/wl/wr value (%s)", text);
		return;
	}
	text += n;
	...
	for (deco = 1, dd = &deco_def_tb[1]; deco < 128; deco++, dd++) {
		if (dd->name == 0) {
		  alert( "Decoration %s unknown",name);
		  return;
		}
	...
	if (ps_x == sizeof ps_func_tb / sizeof ps_func_tb[0]) {
		alert( "Too many postscript functions");
		return;
	}
	...
	if (*text != '\0') {
		...
		if (str_x == sizeof str_tb / sizeof str_tb[0]) {
			alert( "Too many decoration strings");
			return;
		}
	} else	str_x = 0;
	      ...
	      SYMBOL *s)
{
	...
	for (i = dc->n; --i >= 0; ) {
		...
		deco = dc->t[i];
			...
			deco = deco_tune[deco];
			if (deco == 0 && dc->t[i] != 0)
				alert("Notation '%c' not treated (line %d)", dc->t[i],s->linenum);
		} else
...
void deco_update(SYMBOL *s, float dx)
{
	deco_elt *de;
...
char deco_intern(uchar deco)
{
	char *name;
	name = deco_tb[deco - 128];

	for (deco = 1; deco < 127; deco++) {
	...
	if (deco == 128) {
		alert( "Decoration %s not treated", name);
		deco = 0;
	}
	...
	for (s = first_voice->sym; s != 0; s = s->ts_next) {
		...
		for (k = dc->n; --k >= 0; ) {
			...
			if (dd->func >= 3)	/* if not near the note */
				continue;
			if (s->type != NOTE) {
				alert( "Cannot have a %s on a rest or a bar (line %d)",
				       dd->name, s->linenum);
				continue;
			}
			func_tb[dd->func](de);
...
/*fixme: line cut on repeat!*/
			...
			y2 = s1->nxt->dc_top;
			if (s1->nxt->text != 0)	/* if guitar chord */
		...
		repnl = 0;
		for (s = first_repeat; s != 0; s = s->nxt) {
			const char *p;
			float w;
...
/*fixme*/
			if (s1 == s2)
				break;
			x = s1->x;
			if ((s1->un.bar.type & 0x0f) == B_COL)
				x -= 4;
			i = 0;			/* no end of bracket */
			w = s2->x - x - 8;
			if (s2->type != BAR)
				w = realwidth - x - 4;
			else if (((s2->un.bar.type & 0xf0)	/* if complex bar */
				   && s2->un.bar.type != (B_OBRA << 4) + B_CBRA)
				 || s2->un.bar.type == B_CBRA) {
				i =  2;
				if ((s2->un.bar.type & 0x0f) == B_COL)
					w -= 4;
				else if (!(s2->sflags & S_RRBAR)
					 || s2->un.bar.type == B_CBRA
					 || s2->un.bar.type == (B_CBRA << 4) + B_BAR)
			...
			if (i == 0 && s2->nxt == 0) {	/* 2nd ending at end of line */
	...
	if (cfmt.measurenb >= 0) {
		...
		showm = (char*)(cfmt.measurebox ? "showb" : "show");
		any_nb = 0;
...
/*fixme: KO when no bar at the end of the previous line */
		...
		bar_time = (int)(first_voice->sym->time + wmeasure);
		for (s = first_voice->sym; s != 0; s = s->nxt) {
			switch (s->type) {
			case TIMESIG:
				wmeasure = s->un.meter.wmeasure;
				bar_time = (int)(s->time + wmeasure);
				continue;
			case MREST:
			case MREP:
				nbar += s->un.bar.len - 1;
...
/* -- draw guitar chords -- */
/* (the staves are not yet defined) */
static void draw_gchord(SYMBOL *s,
			float gchy)
{
	...
	xmin = xmax = s->type==MREST ? s->x-s->wl : s->x;
	box = cfmt.gchordbox;
...
/*fixme: what symbol space?*/
		...
		case '@': {		/* absolute */
			...
			if (sscanf(p, "%f,%f%n", &xo, &yo, &n) != 2) {
				alert("Error in guitar chord \"@\" format (line %d)",s->linenum);
				y = s->y + yo;
			} else {
				x += xo;
				y = s->y + yo;
	...
	if (box == 2) {		/* if any normal guitar chord */
		PUT5("%.1f \x01%c%5.2f %.1f %.1f box",
		     xmin - 2, '0' + s->staff, ymin - 3,
		     xmax - xmin + 6, ymax - ymin + 2);
	}
	...
	if (any_tempo) {
		...
		if (any_vocal)
			...
			for (s = first_voice->sym; s != 0; s = s->nxt) {
				...
				if (s->un.tempo.str1 != 0) {
					tex_str(tmp, s->un.tempo.str1, sizeof tmp, &w);
					nw += w * cfmt.tempofont.size;
				}
				if (s->un.tempo.value != 0) {
					i = 1;
					while (i < sizeof s->un.tempo.length
							/ sizeof s->un.tempo.length[0]
					       && s->un.tempo.length[i] > 0) {
						nw += 10;
						i++;
					}
					nw += 10 + 10;
				}
				if (s->un.tempo.str2 != 0) {
					tex_str(tmp, s->un.tempo.str2, sizeof tmp, &w);
					nw += w * cfmt.tempofont.size;
				}
		...
		for (s = first_voice->sym; s != 0; s = s->nxt) {
			...
			if (s->un.tempo.str1 != 0) {
				tex_str(tmp, s->un.tempo.str1, sizeof tmp, 0);
				PUT1("(%s) show\n", tmp);
			}
			...
			if (s->un.tempo.value != 0) {
				...
				if (s->un.tempo.length[0] == 0)
					s->un.tempo.length[0] = beat;
				j = 0;
				while (j < sizeof s->un.tempo.length
						/ sizeof s->un.tempo.length[0]
				       && s->un.tempo.length[j] > 0) {
					identify_note(s, s->un.tempo.length[j],
						      &head, &dots, &flags);
					...
					if (s->un.tempo.length[j] < SEMIBREVE) {
						if (flags <= 0)
							PUT1(" %d su", STEM);
						else {
			...
			if (s->un.tempo.str2 != 0) {
				tex_str(tmp, s->un.tempo.str2, sizeof tmp, 0);
				PUT1("(%s) show\n", tmp);
			}
		}
	} else	ht = 0;
...
/*fixme: should reduce if parts don't overlap tempo...*/
	...
	if (any_vocal)
		...
		for (s = first_voice->sym; s != 0; s = s->nxt) {
			if (s->type != PART)
				continue;
			tex_str(tmp, &s->text[2], sizeof tmp, &w);
	...
	for (s = first_voice->sym; s != 0; s = s->nxt) {
		if (s->type != PART)
			continue;
		tex_str(tmp, &s->text[2], sizeof tmp, 0);
