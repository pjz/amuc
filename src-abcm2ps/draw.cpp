...
{
	if (nbuf + 100 > BUFFSZ) {
		alert( "PS output exceeds reserved space per staff -- increase BUFFSZ");
		return;
	}
}
			  ...
			  struct SYMBOL *s1)
{
	...
	two_staves = 0;
	staff = s1->staff;
	stem_xoff = s1->un.note.grace ? GSTEM_XOFF : STEM_XOFF;
	for (s = s1; ; s = s->nxt) {
		s->xs = s->x;
		if (s->type != NOTE)
			continue;
		...
		if (s->staff != staff)
			two_staves = 1;
		if (s->un.note.word_end)
			break;
	}

	if ((s2 = s) == 0) {
		alert("No beam end! (line %d)",s1->linenum);
		return 0;
	}
	...
	if (bm->staff >= 0) {	/* staves not defined */
		...
		if (!two_staves && !s1->un.note.grace)
			return 0;
	}
			 ...
			 struct SYMBOL *s)
{
	...
	if (s->un.meter.wmeasure == 0)
		return;
	x = x - s->wl + 3.5;
	for (i = 0; i < s->un.meter.nmeter; i++) {
		const char *f; char meter[64];
		int l;
		float dx;

		if (s->un.meter.meter[i].top[0] == ' ') {
			x += 6.;			/* half-space */
			continue;
		}
		l = strlen(s->un.meter.meter[i].top);
		if (s->un.meter.meter[i].bot[0] != '\0') {
			int l2;

			sprintf(meter, "(%.8s) (%.2s) ",
				s->un.meter.meter[i].top,
				s->un.meter.meter[i].bot);
			f = "tsig";
			l2 = strlen(s->un.meter.meter[i].bot);
			if (l2 > l)
				l = l2;
		} else {
			if (s->un.meter.meter[i].top[0] == 'C') {
				if (s->un.meter.meter[i].top[1] != '|')
					f = "csig";
				else	f = "ctsig";
				meter[0] = '\0';
			} else if (s->un.meter.meter[i].top[0] == '('
				   || s->un.meter.meter[i].top[0] == ')') {
				sprintf(meter, "(\\%s) ",
					s->un.meter.meter[i].top);
				f = "stsig";
			} else {
			...
			struct SYMBOL *s)
{
	...
	if (s->un.key.nacc == 0) {
		...
		/* sharps */
		} else if (s->un.key.sf > 0) {
			if (s->un.key.sf < old_sf) {
				clef_shift = sharp_cl[clef_ix];
				for (i = s->un.key.sf; i < old_sf; i++) {
					if ((shift = sharp_tb[i] + clef_shift) < -3)
						shift += 21;
					PUT2("%.1f %.1f nt0 ", x, staffb + shift);
					x += 5;
		...
		last_acc = s->un.key.accs[0];
		for (i = 0; i < s->un.key.nacc; i++) {
			const char *p;

			shift = sharp_cl[clef_ix];
			p = "sh0";
			if (s->un.key.accs[i] != last_acc) {
				last_acc = s->un.key.accs[i];
				x += 3;
			}
			switch (last_acc) {
			case A_SH:
	...
	if (old_sf != 0 || s->un.key.sf != 0 || s->un.key.nacc != 0)
		PUT0("\n");
}
...
static const char *rest_tb[9] = {
	...
	if (s->un.note.len == voice_tb[s->voice].meter.wmeasure) {
		struct SYMBOL *prev;
			    ...
			    struct SYMBOL *s)
{
	...
	for (g = s->grace; ; g = g->nxt) {
		if (s->grace->nxt != 0) {	/* many notes */
	...
	if (voice_tb[s->voice].key.bagpipe	/* no slur when bagpipe */
	    || !cfmt.graceslurs
	    || s->un.note.slur_st		/* explicit slur */
	    || s->nxt == 0
	    || s->nxt->type != NOTE)
		return;
	yy = 1000;
	for (g = s->grace; g != 0; g = g->nxt) {
		if (g->y < yy) {
			yy = g->y;
			last = g;
		}
...
static const char *acc_tb[] = { "", "sh", "nt", "ft", "dsh", "dft" };
	...
	identify_note(s, s->un.note.lens[m],
		      &head, &dots, &nflags);
	...
	if (s->un.note.grace)
		p = "ghd";
	else if (s->un.note.accs[m]
		 && voice_tb[s->voice].clef.type == PERC)
		p = "2 copy /y exch def /x exch def dsh0";
	else	{
		switch (head) {
		case H_SQUARE:
			if (s->un.note.lens[m] < BREVE * 2)
				p = "breve";
			else	p = "longa";
			break;
		case H_OVAL:
			if (s->un.note.lens[m] < BREVE)
				p = "HD";
			else	p = "HDD";
			break;
		case H_EMPTY:
		      ...
		      int fl)
{
	...
	if (!s->un.note.stemless) {		/* add stem and flags */
		char	c, c2;
		float	slen;
...
/*	int two_staves; */
	...
	if (!((s1->sflags & S_WORD_ST) || (s1->prv->sflags & S_BEAM_BREAK)
	       || s1->prv->nflags < s1->nflags)
/*	       || s1->prv->un.note.word_end) */
	    || !(s2->un.note.word_end || (s2->sflags & S_BEAM_BREAK)))
		nb_only = 0;
	upstaff = s1->staff;
/*	two_staves = 0; */
	for (sy = s1; ; sy = sy->nxt) {
...
/*			break; */
		...
		if (sy == s2)
			break;
		if (sy->un.note.word_end || (sy->sflags & S_BEAM_BREAK))
			nb_only = 0;
	}
	...
	if (nb_only) {
		...
		PUT4("(%d) %.1f \x01%c%5.2f bnum\n",
		     s1->un.note.p_plet, xm, '0' + s1->staff, ym);
		if (s1->stem > 0) {
			b += 8.;
			for (sy = s1; ; sy = sy->nxt) {
				yy = a * sy->xs + b;
...
/*fixme: the note may be shifted to the right*/
	...
	yy = 0.5 * (y1 + y2);
	PUT4("(%d) %.1f \x01%c%5.2f bnum\n",
	     s1->un.note.p_plet, xm, '0' + upstaff, yy);
}
...
if (two_staves) alert( "*** multi-staves slurs not treated");
...
static struct SYMBOL *next_scut(struct SYMBOL *s)
{
	...
	prev = s;
	for (s = s->nxt; s != 0; s = s->nxt) {
		if (s->type == BAR
		    && (s->un.bar.type == B_RREP
			|| s->un.bar.type == B_DREP
			|| s->un.bar.type == B_THIN_THICK
			|| s->un.bar.type == B_THICK_THIN
			|| (s->un.bar.repeat_bar
			    && s->text != 0
			    && s->text[0] != '1')))
			return s;
...
static void draw_all_ties(struct SYMBOL *sym)
{
	...
	for (s1 = sym; s1 != 0; s1 = s1->nxt)
		if (s1->type != CLEF && s1->type != KEYSIG
		    && s1->type != TIMESIG)
			break;
	for (s2 = s1; s2 != 0; s2 = s2->nxt) {
		if (s2->type == NOTE)
			break;
	}
	if (s2 == 0)
	...
	for (;;) {
		...
		for (s2 = s2->nxt; s2 != 0; s2 = s2->nxt) {
			if (s2->type == NOTE)
				break;
			if (s2->type == BAR
			    && (s2->un.bar.type == B_RREP
				|| s2->un.bar.type == B_DREP
				|| s2->un.bar.type == B_THIN_THICK
				|| s2->un.bar.type == B_THICK_THIN
				|| (s2->un.bar.repeat_bar
				    && s2->text != 0
				    && s2->text[0] != '1')))
				break;
		}
	...
	s2 = next_scut(s1);
	if (s2->type==NOTE)  // this is too rigorous
		draw_ties(s1, s2, 2);
}
...
static void draw_all_slurs(struct SYMBOL *sym)
{
	...
	for (pass = 0; ; pass++) {
		...
		for (;;) {
			...
			for (;;) {
				...
				if (s1->un.note.slur_end) {
					k = s1;
					break;
				}
				if (s1->un.note.slur_st)
					break;
				s1 = s1->nxt;
			}
			if (k == 0) {
			...
			if (gr1 != 0) {
				for (s1 = s; s1->nxt != 0; s1 = s1->nxt)
					;
				s1->nxt = gr1->nxt;
				gr1->nxt->prv = s1;
				gr1->un.note.slur_st = 1;
			}
			if (gr2 != 0) {
				gr2->prv->nxt = gr2->grace;
				gr2->grace->prv = gr2->prv;
				gr2->un.note.slur_st = 1;
			}
	...
	for (s = sym; s != 0; s = s->nxt) {
		if (s->type != NOTE
		    && s->type != REST)
			continue;
		if (s->un.note.slur_end) {
			cut = prev_scut(s);
			draw_slur(cut, s);
		}
		if (s->un.note.slur_st) {
			cut = next_scut(s);
			draw_slur(s, cut);
...
void draw_sym_near(void)
{
	...
	for (s = first_voice->sym; s != 0; s = s->ts_next) {
		...
		if (s->type != NOTE) {
			if ((gr = s->grace) == 0)
				continue;
			for (gr = s->grace; gr->nxt != 0; gr = gr->nxt)
				;
			x = s->x - gr->x;
			for (gr = s->grace; gr != 0; gr = gr->nxt)
				gr->x += x;
			continue;
		}
		nhd = s->un.note.nhd;
		if (s->un.note.accs[nhd]) {
			y = s->y + 8;
			if (s->dc_top < y)
				s->dc_top = y;
		}
		if (s->un.note.accs[0]) {
			y = s->y;
			if (s->un.note.accs[0] == A_SH
			    || s->un.note.accs[0] == A_NT)
				y -= 7;
			else	y -= 5;
			if (s->dc_bot > y)
				s->dc_bot = y;
...
void draw_symbols(struct VOICE_S *p_voice)
{
	...
	for (s = sym; s != 0; s = s->nxt) {
		...
		case CLEF: {
			...
			staff = s->staff;
			memcpy(&staff_tb[staff].clef, &s->un.clef, /* (for next lines) */
				sizeof s->un.clef);
			if (s->un.clef.invis)
				break;
			y = staff_tb[staff].y;

			switch (s->un.clef.type) {
			case BASS:
				ct = 'b';
				y += (float) ((s->un.clef.line - 4) * 6);
				break;
			case ALTO:
				ct = 'c';
				y += (float) ((s->un.clef.line - 3) * 6);
				break;
			case TREBLE:
				y += (float) ((s->un.clef.line - 2) * 6);
				break;
			case PERC:
				ct = 'p';
				break;
			...
			PUT4("%.1f %.1f %c%cclef\n", x, y,
			     s->u ? 's' : ' ', ct);
			if (s->un.clef.octave == 0)
				break;
/*fixme: works fine for treble clef only*/
			PUT3("%.1f %.1f oct%c\n", x, y,
			     s->un.clef.octave > 0 ? 'u' : 'l');
			break;
		}
		case TIMESIG:
		...
		case MREP:
			if (p_voice->second)
				break;
			if (s->un.bar.len == 1) {
				x = (s->prv->x + s->nxt->x) * 0.5;
				PUT2("%.1f %.1f mrep\n",
				     x, staff_tb[s->staff].y);
				break;
			}
			PUT2("%.1f %.1f mrep2\n",
			     s->prv->x, staff_tb[s->staff].y);
			if (s->voice != first_voice - voice_tb)
				break;
			set_font(&cfmt.gchordfont);
			PUT3("%.1f %.1f M (%d) cshow\n",
			     s->prv->x, staff_tb[s->staff].y + 24 + 4,
			     s->un.bar.len);
			break;
		case GRACE:
			draw_gracenotes(x, s);
...
void draw_whistle(void)
{
	...
	int sf;
	char workmap[70];	/* sharps/flats - base: lowest 'C' */
	char basemap[7];
	static char pitnam[12 * 2 + 1] = "C\0C#D\0EbE\0F\0F#G\0AbA\0BbB\0";
	static int w_tb[12] = {
	...
	for (i = 0; i < nwhistle; i++) {
		...
		for (s = p_voice->sym; s != 0; s = s->nxt) {
			...
			if (s->un.note.ti2[0] != 0)	/* tied note */
				continue;
			pitch = s->un.note.pits[0] + 19;
			if (s->un.note.accs[0] != 0) {
				workmap[pitch] = s->un.note.accs[0] == A_NT
					? A_NULL
					: s->un.note.accs[0];
			}
			pitch = scale[pitch % 7]
