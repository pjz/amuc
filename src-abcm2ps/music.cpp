...
static void set_head_directions(struct SYMBOL *s)
{
	...
	for (i = n; i >= 0; i--) {
		...
		for (m = 0; m <= n; m++) {
			...
			if (da <= 5 && xx < xmn)
				xmn = xx;
			if (d > 0 && da < nac && s->un.note.accs[m])
				nac = da;
		}
			...
			int clef_type)
{
	...
	/* create the symbol */
	new_s = ins_sym(CLEF, s->prv);
	new_s->un.clef.type = clef_type;
	new_s->un.clef.line = clef_type == TREBLE ? 2 : 4;
...
static void def_tssym(void)
{
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
		if ((s = p_voice->sym) == 0) {
			alert( "Voice %s is empty", p_voice->name);
			continue;
		}
		s->ts_prev = prev_sym;
		if (prev_sym != 0)
	...
	for (;;) {
		...
		if (t != 0) {
			...
			for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
				...
				else if (s->time != time) { /* misplaced bar */
					ko = 2;
					break;
				}
				p_voice->selected = 1;
			...
			if (ko) {
				int newtime;
				if (ko==1)
					alert("Too many notes in measure %d for voice %s (line %d)",
 						bars, p_voice->name, t->linenum);
				else if (ko==2) 
					alert("Misplaced bar in measure %d (near voice %s) (line %d)",
						bars, p_voice->name,t->linenum);
	...
	while (s != 0) {
		...
		for (t = s;
			...
			if (t->dc_top > stb[staff].st[i].ymx)
				stb[staff].st[i].ymx = (int)t->dc_top;
			if (t->dc_bot < stb[staff].st[i].ymn)
				stb[staff].st[i].ymn = (int)t->dc_bot;
		}
...
/*fixme: may be too high*/
			...
			if (s->dc_top > stb[staff].st[i].ymx)
				stb[staff].st[i].ymx = (int)s->dc_top;
			s->dc_bot = s->y - ls;
			if (s->dc_bot < stb[staff].st[i].ymn)
				stb[staff].st[i].ymn = (int)s->dc_bot;
		}
...
static void set_global(void)
{
		...
		for (s = sym; s != 0; s = s->nxt) {
			...
			case NOTE: {
				...
				if (last_tie != 0) {
					for (i = 0; i <= last_tie->nhd; i++) {
						...
						if (last_tie->un.note.ti1[i] == 0)
							continue;
						pit = last_tie->pits[i];
						last_tie->un.note.ti1[i] = 0;
						for (j = 0; j <= s->un.note.nhd; j++) {
							if (s->pits[j] == pit) {
								last_tie->un.note.ti1[i] = j + 1;
								break;
							}
						}
					}
			...
			case REST:
				...
				if (s->nflags <= 0) {
					...
					s->un.note.word_end = start_flag = 1;
					s->sflags |= S_WORD_ST;
				} else if (s->type == NOTE) {
					if (start_flag)
						s->sflags |= S_WORD_ST;
					if (s->sflags & S_EOLN) {
						s->un.note.word_end = 1;
						start_flag = 1;
					}
					start_flag = s->un.note.word_end;
					lastnote = s;
				} else if (s->un.note.word_end) {
					if (lastnote != 0) {
						lastnote->un.note.word_end = 1;
						lastnote = 0;
					}
					s->un.note.word_end = 0;
					start_flag = 1;
				}
				break;
			}
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next)
		memcpy(&p_voice->sym->un.clef,
		       &staff_tb[p_voice->staff].clef,
		       sizeof p_voice->sym->un.clef);
	set_pitch();
}
	...
	for (s = sym; s != 0; s = s->nxt) {
		...
		if ((s->stem = s->multi) == 0) {
			...
			if ((s->sflags & S_WORD_ST)
			    && !s->un.note.word_end) {	/* start of beam */
				int avg, n;
				struct SYMBOL *t;

				avg = s->yav;
				n = 1;
				for (t = s->nxt; t != 0; t = t->nxt) {
					if (t->type == NOTE) {
						avg += t->yav;
						n++;
					}
					if (t->un.note.word_end)
						break;
				}
				avg /= n;
				stem = 1;
		...
		if (voice_tb[s->voice].key.bagpipe)
			s->stem = -1;
		laststem = (s->len >= MINIM || s->un.note.stemless) ? 0 : s->stem;
		lasty = s->yav;
		if (s->len <= SEMIQUAVER / 2
		    && s->prv->len <= SEMIQUAVER / 2)
			do_break = 1;
	}
...
static void set_stems(void)
{
	...
	for (s = tssym; s != 0; s = s->ts_next) {
		...
		if (s->un.note.stemless) {
			...
			ymin = s->ys - 2.;
			ymax = (float) (s->y + 4);
			if (s->un.note.ti1[s->nhd] != 0
			    || s->un.note.ti2[s->nhd] != 0)
				ymax += 3.;
		}
...
/*fixme: take the length from the tempo */
		...
		if (s->text == 0)
			break;
		tex_str(t, s->text, sizeof t, &w);
		w += cwid(' ') * 1.5;
		xx = cfmt.gchordfont.size * cfmt.gchordfont.swfac * w;
		if (!s->un.bar.repeat_bar) {
			if (s->prv->text != 0) {
				float spc;
...
/*fixme: may have gchord a bit further..*/
		...
		if (s->nxt != 0
		    && s->nxt->text != 0
		    && s->nxt->len > 0) {	/*fixme: what if 2 bars?*/
			AT_LEAST(s->wr, xx);
			s->pr = s->wr;
		}
		break;
	...
	case KEYSIG: {
		...
		if (n1 > 0) {
			...
			s->pl = s->wl = 2;
		} else if (s->nxt != 0 && s->nxt->type != TIMESIG)
			s->pl = s->wl = 2;
		break;
	}
	case TIMESIG:
	...
	case MREP:
		if (s->un.bar.len == 1) {
			...
			s2 = s->prv->prv;	/* invisible rest (see parse) */
			s->wl = s2->wl;
			s->wr = s2->wr;
			s->pl = s2->pl;
			s->pr = s2->pr;
...
static void check_bar(struct SYMBOL *s)
{
	...
	if (s->type == KEYSIG && s->prv != 0 && s->prv->type == BAR)
		s = s->prv;
	if (s->type != BAR)
		return;
	if (s->un.bar.repeat_bar) {
	...
	if (!(s->sflags & S_RRBAR)) {		/* 'xx:' (not ':xx:') */
		...
		if (s->prv != 0 && s->prv->type == BAR) {
			s->prv->nxt = 0;
			if (s->ts_prev != 0)
				s->ts_prev->ts_next = s->ts_next;
			if (s->ts_next != 0)
				s->ts_next->ts_prev = s->ts_prev;
		} else	s->un.bar.type = B_BAR;
		return;
	}
	if (bar_type == B_DREP) {		/* '::' */
		...
		for ( ; s != s2; s = s->ts_next) {
			...
			if (s->nxt != 0) {
				...
				if (s->nxt->type == BAR
				    ...
				    && s->nxt->nxt->text == 0) {
					s3 = 0;
					if ((s->nxt->un.bar.type == B_SINGLE
					     || s->nxt->un.bar.type == B_DOUBLE)
					    && (s->nxt->nxt->un.bar.type & 0xf0))
						s3 = s->nxt;
					if (s3 != 0) {
						s3->prv->nxt = s3->nxt;
						if (s3->nxt != 0)
...
/*fixme: may also have a meter change*/
	...
	if (space > width) {
		...
		if (beta > beta0) {
			if (!cfmt.continueall) {
				alert("Line %d underfull (%.0fpt of %.0fpt)", s->linenum,
	...
	if (first_voice->next == 0) {
		for (s = first_voice->sym; s->nxt != 0; s = s->nxt) {
...
static void init_music_line(struct VOICE_S *p_voice)
{
	...
	/* add clef */
	s = add_sym(p_voice, CLEF); // (parse.c)
	memcpy(&p_voice->clef, &staff_tb[p_voice->staff].clef,
	       sizeof p_voice->clef);
	memcpy(&s->un.clef, &p_voice->clef,
	       sizeof s->un.clef);

	/* add keysig */
	s = add_sym(p_voice, KEYSIG);
	s->seq++;
	memcpy(&s->un.key, &p_voice->key, sizeof s->un.key);
	if (s->un.key.bagpipe && s->un.key.sf == 2)	/* K:Hp */
		s->u = 3;				/* --> Gnat */
	...
	if (insert_meter
		...
		s->seq += 2;
		memcpy(&s->un.meter, &first_voice->meter,
		       sizeof s->un.meter);
	}
	...
	if (p_voice->bar_start != 0) {
		...
		if (p_voice->bar_text == 0	/* if repeat continuation */
			...
			for (s = sym; s != 0; s = s->nxt) {	/* search the end of repeat */
				if (s->type == BAR) {
					if ((s->un.bar.type & 0xf0)	/* if complex bar */
					    || s->un.bar.type == B_CBRA
					    || s->un.bar.repeat_bar)
						break;
					if (--i < 0)
						break;
		...
		if (i >= 0) {
			...
			s->seq += 3;
			s->un.bar.type = p_voice->bar_start;
			s->text = p_voice->bar_text;
			s->un.bar.repeat_bar = p_voice->bar_repeat;
		}
		p_voice->bar_start = 0;
		p_voice->bar_repeat = 0;
		p_voice->bar_text = 0;
	}
	if ((p_voice->last_symbol->nxt = sym) != 0)
		sym->prv = p_voice->last_symbol;
	p_voice->nvocal = 0;
}
